# [BetterDiscord](https://github.com/rauenzi/BetterDiscordApp) [![Support][support-badge]][support-link] [![PayPal][paypal-badge]][paypal-link]

[support-badge]: https://img.shields.io/badge/Support-%2343b581.svg?style=flat&logo=data:image/png;base64,
[support-link]: https://discord.com/invite/kHe2KNzygB

[paypal-badge]: https://img.shields.io/badge/PayPal-%23003087.svg?style=flat&logo=data:image/png;base64,
[paypal-link]: https://paypal.me/kloperox?locale.x=pl_PL

# Using

Open Discord, go to Settings > Plugins, enable StatusAnimated v0.0.1 by !!Klopero and click Settings.
Enter the required information in the fields and click Save

Copyright (C) 2021 !!Klopero
